...
def gen_duanluqi_excel():
    f_excel = this_root + u'190714\\张桥所线路设备明细.xls'
    bk = xlrd.open_workbook(f_excel)
    sh = bk.sheet_by_name(u'柱上断路器')
    nrows = sh.nrows
    rongduanqi = {}
    for i in range(nrows):
        rongduanqi_name = sh.cell_value(i, 0)
        rongduanqi_attrib = sh.cell_value(i, 2)
...
def gen_zhuanbian_excel():
    f_excel = this_root + u'190714\\张桥所线路设备明细.xls'
    bk = xlrd.open_workbook(f_excel)
    sh = bk.sheet_by_name(u'柱上用户变压器')
    nrows = sh.nrows
    zhuanbian = {}
    for i in range(nrows):
        if i+1 == nrows:
            continue
        bianyaqi_name = sh.cell_value(i+1, 13)
        bianyaqi_yonghu = sh.cell_value(i+1, 8)
        bianyaqi_rongliang = str(int(sh.cell_value(i+1, 5)))
        bianyaqi_xinghao = sh.cell_value(i+1, 6)
...
def line_annotation():
    ...
    :return:
    '''
    npy = this_root + '190714\\shp_dic.npy'
    ...
    for key in xian_dic_sort:
        ...
        dwg_dir = this_root+u'190714\\dwg_to_shp\\'+val2.decode('utf-8')+'\\annotation\\'
        mk_dir(dwg_dir)
...
def gen_shp_dic():
    ...
    fdir = this_root+u'190714\\dwg_to_shp\\'
    ...
    np.save(this_root+'190714\\shp_dic',shp_dic)
        # exit()
...
def merge_point_shp(shp_type):
    ...
    for folder in flist:
        ...
        time_i += 1
        log_process.process_bar(time_i,len(flist),time_init,time_start,time_end)
    output_fn = this_root + '190714\\'+shp_type+'_merge.shp'.decode('gbk').encode('utf-8')
...
if __name__ == '__main__':
    # main()
    merge_point_shp('zhuanbian')
    # shptypes = ['duanluqi','gongbian','naizhang_ganta','zhuanbian']
